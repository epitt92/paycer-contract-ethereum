        ...
        returns (uint256 pending)
    {
        ...
        if (block.timestamp > user.lastRewardTime && user.amount != 0) {
            ...
            accRewardPerShare_ = accRewardPerShare_.add(
                timeReward / user.amount
            );
        }
        pending = (
            (user.amount.mul(accRewardPerShare_).div(ACC_REWARD_PRECISION))
                .toInt256()
                .sub(user.rewardDebt)
        ).toUint256();
    }
        ...
        if (block.timestamp > user.lastRewardTime) {
            if (user.amount > 0) {
                ...
                uint256 timeReward = user
                    ...
                    .div(APY_ACCURACY)
                    .div(365 days);
                user.accRewardPerShare = user.accRewardPerShare.add(
                    timeReward.div(user.amount)
                );
            }
